...
namespace SimpleMCMain4
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            uint option_type;
            Console.Write("Enter 0 for Call, otherwise Put : ");
            option_type = uint.Parse(Console.ReadLine());
            PayOff payoff;
            if(option_type == 0){
                payoff = new PayOffCall(strike);
            }
            else
            {
                payoff = new PayOffPut(strike);
            }
